#!/Users/hectorlopezhernandez/PycharmProjects/chat/myenv/bin/python3
...
import argparse
import sys
sys.path.append('/Users/hectorlopezhernandez/PycharmProjects/stable_diffusion/ml-stable-diffusion/python_coreml_stable_diffusion')
...
class CoreMLStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.VAE_DECODER_UPSAMPLE_FACTOR = 8
        ...
        self.height = latent_h * self.VAE_DECODER_UPSAMPLE_FACTOR
        self.width = latent_w * self.VAE_DECODER_UPSAMPLE_FACTOR
    ...
    def run_controlnet(self,
        ...
        for i, (module, cond) in enumerate(zip(self.controlnet, controlnet_cond)):
            module_outputs = module(
                ...
                controlnet_cond=cond.astype(np.float32),
            )
            if i == 0:
        ...
        if self.safety_checker is not None:
            ...
            safety_checker_outputs = self.safety_checker(
                ...
                adjustment=np.array([0.]).astype(
                    np.float32),  # defaults to 0 in original pipeline
            )
    ...
    def decode_latents(self, latents):
        ...
        path = '/Users/hectorlopezhernandez/PycharmProjects/chat/preclipped'
        fi = 0
        while os.path.isfile(path + '.npy'):
            fi += 1
            path = path + f'_{fi}'
    ...
    def __call__(
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            latents = self.scheduler.step(torch.from_numpy(noise_pred.astype(np.float16)),
                                          t,
                                          torch.from_numpy(latents.astype(np.float16)),
                                          **extra_step_kwargs,
                                          ).prev_sample.numpy()
            ...
            if i%100==0 and not i==0:
                self.numpy_to_pil(self.decode_latents(latents))[0].show()
...
def get_image_path(args, **override_kwargs):
    ...
    """
    args.prompt = args.prompt[:50]
    out_folder = os.path.join(args.o, "_".join(args.prompt.replace("/", "_").rsplit(" ")))
    os.makedirs(out_folder, exist_ok=True)

    out_fname = f"_randomSeed_{override_kwargs.get('seed', None) or args.seed}_"
    out_fname += time.strftime("%d-%H:%M:%S", time.localtime())
